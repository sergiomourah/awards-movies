...
import {
  getIntervalWinsMaxMin,
  getListWinnersByYear,
  getListYears,
  getTopStudios,
...
const DashboardContainer = () => {
  ...
  const labelsListYears = [
    { column: "year", name: t("dashboard.listYears.year") },
    { column: "winnerCount", name: t("dashboard.listYears.winnerCount") },
  ];

  const labelsTopStudios = [
    { column: "name", name: t("dashboard.topStudios.name") },
    { column: "winCount", name: t("dashboard.topStudios.winCount") },
  ];

  const labelsInterval = [
    { column: "producer", name: t("dashboard.intervalWinsMaxMin.producer") },
    { column: "interval", name: t("dashboard.intervalWinsMaxMin.interval") },
    {
      column: "previousWin",
      name: t("dashboard.intervalWinsMaxMin.previousWin"),
    },
    {
      column: "followingWin",
      name: t("dashboard.intervalWinsMaxMin.followingWin"),
    },
  ];

  const rowsIntervalMax = (intervalWinsMaxMin.max ?? []).map(
  ...
  const rowsIntervalMin = (intervalWinsMaxMin.min ?? []).map(
    ({ producer, interval, previousWin, followingWin }) => ({
      producer,
      interval,
      previousWin,
  ...
  const labelsListWinners = [
    { column: "id", name: t("dashboard.listWinners.id") },
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <div className="flex-row align-center container">
              <span>
                <b>{t("dashboard.listYears.title")}</b>
              </span>
            </div>
          ...
          <Grid item xs={12} sm={6}>
            <div className="flex-row align-center container">
              <span>
                <b>{t("dashboard.topStudios.title")}</b>
              </span>
            </div>
          ...
          <Grid item xs={12} sm={6}>
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <div className="flex-row align-center container">
                  <span>
                    <b>{t("dashboard.intervalWinsMaxMin.title")}</b>
                  </span>
                </div>
                <div className="flex-row align-center container">
                  <span>Maximum</span>
          ...
          <Grid item xs={12} sm={6}>
            <div className="flex-row align-center container">
              <span>
                <b>{t("dashboard.listWinners.title")}</b>
              </span>
            </div>
